...
const DAOQuery = ({ publicAddress, width, height, format }) => {
  ...
  if (error)
    return null; /* <div className="empty failure">{parser(i18n.t('failure-short', { errorMessage: error }))}</div>; */
  ...
  if (!daoTitle) {
    ...
    label =
      daoTitle.length > gui.MAX_LENGTH_ACCOUNT_NAMES
        ? `${daoTitle.slice(0, gui.MAX_LENGTH_ACCOUNT_NAMES)}...`
        : daoTitle;
  }
    ...
    <div className="dao">
      {format === 'plainText' ? (
        <div>
          <img
            src={image}
            className="symbol dao-pic dao-pic-sidebar"
            alt=""
            style={{ width: finalWidth, height: finalHeight }}
          />
          <div className="identity-peer">
            {label.length > gui.MAX_LENGTH_ACCOUNT_NAMES
              ? `${label.substring(0, gui.MAX_LENGTH_ACCOUNT_NAMES)}...`
              : label}
          </div>
        </div>
      ) : (
        <div className="avatar-editor">
          ...
          <div className="identity-peer">
            <Link
              to={url}
              title={publicAddress}
              className="identity-label identity-label-micro identity-label-dao"
              onClick={e => {
                e.stopPropagation();
              }}
            >
