      ...
      <div id="modalToggle" className="modal">
        <div className="alert">
          <div className="alert-header">
            ...
            <div className="modal-title">{parser(this.props.modal.title)}</div>
          </div>
          <div>{parser(this.props.modal.message)}</div>
          {this.props.modal.displayBallot ? (
            <p>
              <ul className="options options-mini">
                <li className="title-input title-input-mini">{this.props.modal.proposalTitle}</li>
              </ul>
            </p>
          ) : null}
          {this.props.modal.mode === 'AWAIT' ? (
            <div className="modal-spinner">
              <SyncLoader size={10} margin={2} color={'var(--menu-sidebar-selected)'} loading={true} />
            </div>
          ) : null}
          {this.props.modal.mode === 'ALERT' ? (
            <div className="modal-buttons">
              <div id="cancel" className="button login-button" onClick={this.cancel}>
                <div>{this.props.modal.cancelLabel}</div>
              </div>
            </div>
          ) : null}
